...
echo $red Delete the controller and worker compute instances:
...
#Delet Networking
echo $red Delete the external load balancer network resources:
{
...
echo $red Delete the k8_glcloud_EZ firewall rules:
...
#Delete network vpc
echo $red Delete the k8_glcloud_EZ network VPC:
{
...
echo $green say bye bye by to all your .pem, .json, .csr, .yaml, and .kubeconfig files 
